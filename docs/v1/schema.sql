-- CPA grouped billing V1 — REFERENCE ONLY, not a production migration.
-- Requires the existing v14 api_keys / request_events tables.
-- Apply only to a disposable test database. Do not open Keeper's database here.
-- Real implementation: one migration transaction, backups, version checks,
-- rollback tests; no change to PRAGMA user_version in this reference.
...
-- [auth_type, identity] for upstream. Not a masked Key, email or plaintext key.
