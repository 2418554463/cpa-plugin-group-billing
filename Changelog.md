...
- 本版仍为 Linux x86_64 / glibc 预发布包；插件商店请手动选择 `v1.3.14-group.1`，不要选择 Latest。
- 从 v1.3.13-group.1 升级不改变计费规则或数据库结构；升级前备份动态库与数据库，替换后重启 CPA。

### 前端

- 资源组编辑拆分为基本信息、模型范围和凭证池，保存按钮固定在底部。
- 模型、供应商及凭证支持搜索勾选与已选数量，保留手动模型 ID 和别名输入。
- 资源组增加状态和模型标签，计划额度与并发配置适配桌面双列和手机单列。
- 优化深浅主题、窄屏滚动及已发布模型的只读展示。

## v1.3.13-group.1

### 升级须知

- Linux x86_64 / glibc 预发布包，尚未做 Linux 动态加载或 Docker 实机验证；正式开放用户前须在目标服务器验收。
- 替换原 `cpa-key-billing` 插件，不可同时加载两份；升级前停机备份数据库，v15 回退旧版须恢复升级前备份。
- 仅支持单 CPA 实例、每 Key 独立额度；计划绑定于北京时间次日零点生效，usage 后结算可能因在途请求超额。
- Keeper 共享备注使用其独立管理密码和 API；不要把 CPA 管理密钥作为 Keeper 密码，也不要运行上游一键更新。

### 后端

- 支持任意模型资源组及供应商/凭证池，每 Key × 组分别设置每日 USD 限额和同时并发，支持显式不限。
- 新增分组订阅、日账本、抵扣式重置、权限交集和审计，保留旧套餐及历史事件。
- 通过 Keeper 共享 Key 与凭证备注，支持按需同步、手动刷新和修改冲突提示。
- 发布完整安装包、动态库 ZIP、源码包及 SHA-256 校验文件。

### 前端

- 新增“分组订阅”管理页，支持动态组、计划、次日绑定、用量、备注与审计，并适配手机宽度。
- 普通 Key 用户可查看自己的分组额度；退出登录后清理分组页数据。
